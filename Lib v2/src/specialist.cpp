...
Checklist::Ptr Specialist::take_check_list(int checklist_id, QSqlDatabase db)
{
    QSqlQuery query(db);
    ...
    //названия полей и таблиц в запросах нужно проверить
       query.prepare("SELECT checklist_id, checklist_name, user_id, equip_type"
                     "FROM check_lists"
                     "WHERE checklist_id=:checklist_id"
                     );
       query.bindValue(":checklist_id",checklist_id);
